...
function StartProgressBar()
    if Config.framework == 'qbcore' then
        QBCore.Functions.Progressbar("fueling_vehicle", Config.Strings.Refueling, Config.ProgressTime * 1000, false,
            true, {
...
RegisterNUICallback('checkMoney', function(data, cb)
    TriggerCallback('DP-Fuel:callback:hasMoney', function(result)
        if result then
            lastAmount = data
            toggleNuiFrame(false)
            StartFueling()
...
function grabNozzleFromPump()
    ...
    AttachEntityToEntity(nozzle, ped, GetPedBoneIndex(ped, 0x49D9), 0.11, 0.02, 0.02, -80.0, -90.0, 15.0, true, true,
        false, true, 1, true)
    RopeLoadTextures()
    ...
    AttachEntitiesToRope(rope, pumpObject, nozzle, pump.x, pump.y, pump.z + 1.45, nozzlePos.x, nozzlePos.y, nozzlePos.z,
...
Citizen.CreateThread(function()
    local enZona = false
    local EntraoenZona = false
    while true do
        enZona = false
        local sleep = 1000
        local ped = PlayerPedId()
        if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0)) then
            if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped and
                GetEntityModel(GetVehiclePedIsIn(PlayerPedId())) ~= -1963629913 then
            else
                local vehicle = GetPlayersLastVehicle()
                local vehicleCoords = GetEntityCoords(vehicle)

                if DoesEntityExist(vehicle) and #(GetEntityCoords(ped) - vehicleCoords) < 2.5 and
                    GetEntityModel(vehicle) ~= -1963629913 then
			...
			if not EntraoenZona and enZona then
				exports['DP-TextUI']:Show(Config.Strings.EToRefuel)
				EntraoenZona = true
			elseif EntraoenZona and not enZona then
				exports['DP-TextUI']:Hide()
				EntraoenZona = false
			end
        else
            Citizen.Wait(250)
